...
class InfluenceGraphv4: # 2nd order approaches (Correlation Based)
    ...
    def update_graph_mat_oneshot(self, locations_x, locations_y, lossmult, trainlossdiff_flat, batchlossdiff_flat, class_normalizer=1):
        #  flattening in row-major order: (iterate through all columns for each row)
        #  Assume weights are already flattened
        shape = (self.node_size, self.node_size)
        
        lossmult_mat = csr_matrix((lossmult, (locations_x,locations_y)), shape=shape)
        trainlossdiff_graph = csr_matrix((trainlossdiff_flat, (locations_x,locations_y)), shape=shape)
        batchlossdiff_graph = csr_matrix((batchlossdiff_flat, (locations_x,locations_y)), shape=shape)
        
        trainlossdiff_squared_graph = csr_matrix((trainlossdiff_flat**2, (locations_x,locations_y)), shape=shape)
        batchlossdiff_squared_graph = csr_matrix((batchlossdiff_flat**2, (locations_x,locations_y)), shape=shape)

        count_mat = csr_matrix((class_normalizer*np.ones(len(locations_x)), (locations_x,locations_y)), shape=shape)

        self.lossmult_sum += lossmult_mat
        self.passive_sum += trainlossdiff_graph
        self.active_sum += batchlossdiff_graph
        self.passive_square_sum += trainlossdiff_squared_graph
        self.active_square_sum += batchlossdiff_squared_graph
        self.lossmult_counts += count_mat
    ...
    def update_influence_graph(self, batch_indices, batch_lossdiff, train_lossdiff):
        ...
        if self.clipping:
            trainlossdiff_mat = np.clip(trainlossdiff_mat, -1 if self.negative_clipping else 0, 1)
        ...
        self.update_graph_mat_oneshot(
            ...
            trainlossdiff_mat,
            batchlossdiff_mat,
            len(batch_indices) if self.class_normalize else 1
        )
