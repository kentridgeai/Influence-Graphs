...
if __name__ == "__main__":
    ...
    # Snapshot folder to load model from
    snapshot_dir = dataset + '_snapshots'

    with open(os.path.join(snapshot_dir, "config_bundle.pkl"), 'rb') as handle:
    ...
    logger.log("Dataloaders generated, starting snapshot batchwise influence estimation...", level=1)
    ...
    for fname in model_files:
        ...
        logger.log("Loaded model using filename: {}".format(fname), level=1)

        # Extract the epoch number from the filename
        match = re.search(r"model_epoch_(\d+)\.pt", fname)
        if match:
            epoch_tag = f"epoch_{match.group(1)}"
        else:
        ...
        npz_path = os.path.join(snapshot_dir, f"{epoch_tag}.npz")
        sparse.save_npz(npz_path, graphmat)

        # ➤ Save metadata
        pkl_path = os.path.join(snapshot_dir, f"{epoch_tag}.pkl")
        ...
        logger.log(f"Saved: {epoch_tag}.npz and {epoch_tag}.pkl", level=1)
